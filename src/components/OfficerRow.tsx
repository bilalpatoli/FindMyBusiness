...
function formatAddress(a: NonNullable<EnrichResponse["person"]>["addresses"] extends (infer T)[] | undefined ? T : never) {
  const parts = [
...
export function OfficerRow({ officer }: { officer: FlattenedOfficer }) {
  const [state, setState] = useState<EnrichState>({ loading: false });
  ...
  const person = state.result?.person;
  const noMatch = state.result && !person;
  ...
  return (
    <div className="border border-zinc-200 dark:border-zinc-800 rounded-md p-3">
      <div className="flex items-start justify-between gap-3">
        ...
        >
          {state.loading ? "Enriching…" : state.result ? "Re-enrich" : "Enrich"}
        </button>
      </div>
      ...
      {person && (
        <div className="mt-3 border-t border-zinc-200 dark:border-zinc-800 pt-3 space-y-3 text-sm">
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-zinc-600 dark:text-zinc-400">
            {person.age !== undefined && <span>Age: <span className="text-zinc-900 dark:text-zinc-100 font-medium">{person.age}</span></span>}
            {state.result?.identityScore !== undefined && (
              <span>Match score: <span className="text-zinc-900 dark:text-zinc-100 font-medium">{state.result.identityScore}</span></span>
            )}
